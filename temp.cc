...
void TabletImpl::UpdateTTL(RpcController* ctrl, const ::openmldb::api::UpdateTTLRequest* request,
    ...
    ::openmldb::api::TTLType ttl_type = request->type();
    uint64_t abs_ttl = 0;
    uint64_t lat_ttl = 0;
    if (request->has_ttl_desc()) {
        ...
        if (ttl_type == ::openmldb::api::TTLType::kAbsoluteTime) {
    ...
    ::openmldb::storage::TTLSt ttl_st(abs_ttl * 60 * 1000, lat_ttl,
                                      ::openmldb::storage::TTLSt::ConvertTTLType(ttl_type));
    ...
    if (abs_ttl > FLAGS_absolute_ttl_max || lat_ttl > FLAGS_latest_ttl_max) {
        ...
        PDLOG(WARNING,
              ...
              abs_ttl, lat_ttl, ::openmldb::api::TTLType_Name(ttl_type).c_str(), FLAGS_absolute_ttl_max,
    ...
    if (request->has_ts_name() && request->ts_name().size() > 0) {
        ...
        table->SetTTL(::openmldb::storage::UpdateTTLMeta(ttl_st, iter->second));
        PDLOG(INFO,
        ...
        table->SetTTL(::openmldb::storage::UpdateTTLMeta(ttl_st));
...
void TabletImpl::Get(RpcController* controller, const ::openmldb::api::GetRequest* request,
    ...
    const ::openmldb::api::TableMeta& table_meta = query_its.begin()->table->GetTableMeta();
...
void TabletImpl::Update(RpcController* controller, const ::openmldb::api::UpdateRequest* request,
                        ::openmldb::api::UpdateResponse* response, Closure* done) {
    brpc::ClosureGuard done_guard(done);
    ...
    if (!ok) {
        if (code == openmldb::base::ReturnCode::kIdxNameNotFound ||
            code == openmldb::base::ReturnCode::kColNameNotFound) {
            response->set_code(code);
            response->set_msg(msg);
        } else {
            response->set_code(::openmldb::base::ReturnCode::kUpdateFailed);
...
int TabletImpl::CheckTableMeta(const openmldb::api::TableMeta* table_meta, std::string& msg) {
    ...
    ::openmldb::api::TTLType type = ::openmldb::api::TTLType::kAbsoluteTime;
    if (table_meta->has_ttl_desc()) {
        ...
        if ((type == ::openmldb::api::TTLType::kAbsoluteTime && ttl > FLAGS_absolute_ttl_max) ||
            (type == ::openmldb::api::kLatestTime && ttl > FLAGS_latest_ttl_max)) {
            uint32_t max_ttl =
                type == ::openmldb::api::TTLType::kAbsoluteTime ? FLAGS_absolute_ttl_max : FLAGS_latest_ttl_max;
        ...
        for (const auto& column_desc : table_meta->column_desc()) {
            ...
            if (column_desc.is_ts_col()) {
                ...
                if (column_desc.has_abs_ttl() || column_desc.has_lat_ttl()) {
                    ...
                    if ((type == ::openmldb::api::TTLType::kAbsoluteTime && ttl > FLAGS_absolute_ttl_max) ||
                        (type == ::openmldb::api::kLatestTime && ttl > FLAGS_latest_ttl_max)) {
                        uint32_t max_ttl = type == ::openmldb::api::TTLType::kAbsoluteTime ? FLAGS_absolute_ttl_max
                                                                                           : FLAGS_latest_ttl_max;
                        msg = "ttl is greater than conf value. max ttl is " + std::to_string(max_ttl);
                        return -1;
                    }
...
void TabletImpl::Traverse(RpcController* controller, const ::openmldb::api::TraverseRequest* request,
    ...
    if (table) {
        ...
        if (!index_def || !index_def->IsReady()) {
            PDLOG(WARNING, "idx name %s ts_index %d not found in table. tid %u, pid %u", index_name.c_str(), ts_index,
                  request->tid(), request->pid());
            response->set_code(::openmldb::base::ReturnCode::kIdxNameNotFound);
            response->set_msg("idx name not found");
            return;
        ...
        openmldb::storage::RelationalTableTraverseIterator* it = nullptr;
        ...
        if (it == NULL) {
            ...
            if (request->has_snapshot_id()) {
                response->set_code(openmldb::base::ReturnCode::kSnapshotRecycled);
        ...
        std::vector<openmldb::base::Slice> value_vec;
        uint32_t total_block_size = 0;
        for (; it->Valid(); it->Next()) {
            if (request->limit() > 0 && scount > request->limit() - 1) {
                DEBUGLOG("reache the limit %u", request->limit());
        ...
        for (const auto& value : value_vec) {
            openmldb::codec::Encode(value.data(), value.size(), rbuffer, offset);
            offset += (4 + value.size());
        }
...
void TabletImpl::BatchQuery(RpcController* controller, const openmldb::api::BatchQueryRequest* request,
                            openmldb::api::BatchQueryResponse* response, Closure* done) {
    brpc::ClosureGuard done_guard(done);
    uint32_t tid = request->tid();
    ...
    if (!ok) {
        if (code == openmldb::base::ReturnCode::kIdxNameNotFound) {
            ...
            response->set_code(::openmldb::base::ReturnCode::kQueryFailed);
...
void TabletImpl::GetTableStatus(RpcController* controller, const ::openmldb::api::GetTableStatusRequest* request,
    ...
    for (auto it = tables_.begin(); it != tables_.end(); ++it) {
        ...
        for (auto pit = it->second.begin(); pit != it->second.end(); ++pit) {
            ...
            ::openmldb::api::TTLDesc* ttl_desc = status->mutable_ttl_desc();
            ::openmldb::storage::TTLSt ttl = table->GetTTL();
            ...
            if (status->ttl_type() == ::openmldb::api::TTLType::kLatestTime) {
...
void TabletImpl::LoadTable(RpcController* controller, const ::openmldb::api::LoadTableRequest* request,
    ...
    do {
        ...
        std::string msg;
        if (!table_meta.has_table_type() || table_meta.table_type() == ::openmldb::type::kTimeSeries) {
        ...
        } else if (table_meta.table_type() == ::openmldb::type::kRelational) {
...
int TabletImpl::LoadDiskTableInternal(uint32_t tid, uint32_t pid, const ::openmldb::api::TableMeta& table_meta,
    ...
    do {
        ...
        if (binlog.RecoverFromBinlog(table, snapshot_offset, latest_offset)) {
            ...
            if (table_meta.has_table_type() && table_meta.table_type() == openmldb::type::kRelational) {
...
int TabletImpl::LoadRelationalTableInternal(const ::openmldb::api::TableMeta& table_meta,
                                            std::shared_ptr<::openmldb::api::TaskInfo> task_ptr) {
    do {
        std::string msg;
...
int32_t TabletImpl::DeleteRelationalTableInternal(uint32_t tid, uint32_t pid,
    ...
    do {
        ...
        ::openmldb::common::StorageMode sm = table->GetStorageMode();
...
void TabletImpl::CreateTable(RpcController* controller, const ::openmldb::api::CreateTableRequest* request,
    ...
    if (table_meta->has_table_type() && table_meta->table_type() == openmldb::type::kRelational) {
        ...
        gc_pool_.DelayTask(FLAGS_snapshot_ttl_check_interval * 60 * 1000,
                           boost::bind(&TabletImpl::GcTableSnapshot, this, tid, pid));
    } else if (table_meta->storage_mode() != openmldb::common::kMemory) {
        std::string msg;
    ...
    if (!table_meta->has_table_type() || table_meta->table_type() == ::openmldb::type::kTimeSeries) {
        ...
        if (table_meta->format_version() == 1 && table_meta->storage_mode() == ::openmldb::common::kMemory) {
        ...
        PDLOG(INFO,
              ...
              ::openmldb::api::TTLType_Name(table_meta->ttl_desc().ttl_type()).c_str());
        gc_pool_.DelayTask(FLAGS_gc_interval * 60 * 1000, boost::bind(&TabletImpl::GcTable, this, tid, pid, false));
    } else {
        std::lock_guard<SpinMutex> spin_lock(spin_mutex_);
...
int32_t TabletImpl::GetSnapshotOffset(uint32_t tid, uint32_t pid, openmldb::common::StorageMode sm, std::string& msg,
...
int TabletImpl::CreateDiskTableInternal(const ::openmldb::api::TableMeta* table_meta, bool is_load, std::string& msg) {
    std::vector<std::string> endpoints;
    ::openmldb::api::TTLType ttl_type = table_meta->ttl_type();
    if (table_meta->has_ttl_desc()) {
        ttl_type = table_meta->ttl_desc().ttl_type();
    }
    if (ttl_type == ::openmldb::api::kAbsAndLat || ttl_type == ::openmldb::api::kAbsOrLat) {
...
int TabletImpl::CreateRelationalTableInternal(const ::openmldb::api::TableMeta* table_meta, bool is_load,
                                              std::string& msg) {
    uint32_t tid = table_meta->tid();
    uint32_t pid = table_meta->pid();
    std::string db_root_path;
...
void TabletImpl::DropTable(RpcController* controller, const ::openmldb::api::DropTableRequest* request,
    ...
    do {
        if (!request->has_table_type() || request->table_type() == ::openmldb::type::kTimeSeries) {
            std::shared_ptr<Table> table = GetTable(tid, pid);
            if (!table) {
                response->set_code(::openmldb::base::ReturnCode::kTableIsNotExist);
                response->set_msg("table is not exist");
...
void TabletImpl::DeleteIndex(RpcController* controller, const ::openmldb::api::DeleteIndexRequest* request,
    ...
    std::string root_path;
    if (!ChooseDBRootPath(tid, pid, ::openmldb::common::StorageMode::kMemory, root_path)) {
        response->set_code(::openmldb::base::ReturnCode::kFailToGetDbRootPath);
        response->set_msg("fail to get table db root path");
        PDLOG(WARNING, "table db path is not found. tid %u, pid %u", tid, pid);
        return;
...
void TabletImpl::RunRequestQuery(RpcController* ctrl, const openmldb::api::QueryRequest& request,
                                 ::fesql::vm::RequestRunSession& session, openmldb::api::QueryResponse& response,
                                 butil::IOBuf& buf) {
    if (request.is_debug()) {
        session.EnableDebug();
    }
